...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        user = User.objects.create(username=validated_data['username'],role=validated_data['role'])
...
class AccountDepositSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if 'deposit' not in data:
            raise serializers.ValidationError("Endpoint has been called without providing deposit value")
        if data['deposit'] not in ALLOWED_COINS_TO_DEPOSIT:
            raise serializers.ValidationError("Endpoint has been called with invalid deposit amount value shloud be  5, 10, 20, 50 or 100.")
        return data
